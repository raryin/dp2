...
namespace DP2PHPClient
{
	...
	public class UnitTests
	{
		...
		public void TestAddReceipt ()
		{
			...
			int lenStock = p.RequestStockInfo(-1).Count;

			StockRecord aRecord = p.RequestStockInfo(-1)[lenStock];
			StockRecord bRecord = p.RequestStockInfo(-1)[lenStock-1];
			StockRecord cRecord = p.RequestStockInfo(-1)[lenStock-2];
			...
			int lenReceiptNew = p.RequestReceiptInfo(-1).Count;

			ReceiptRecord lastReceipt = p.RequestReceiptInfo(-1)[lenReceiptNew];
		...
		public void TestDeleteReceipt ()
		{
			...
			int lenReceipt = p.RequestStockInfo(-1).Count;
			...
			ReceiptRecord aReceipt = p.RequestReceiptInfo(-1)[lenReceiptNew];

			Assert.IsTrue(p.DeleteReceipt(aReceipt.SaleID));

			int lenReceiptDeleted = p.RequestReceiptInfo(-1).Count;
			Assert.IsFalse(lenReceiptNew==lenReceiptDeleted);

			ReceiptRecord lastReceipt = p.RequestReceiptInfo(-1)[lenReceiptDeleted];
